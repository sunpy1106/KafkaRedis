...
/**
 * 测试类：使用 TCP 代理触发 "batch creation" 超时
 *
 * 策略:
 ...
 *  2. 然后运行此测试类
 */
public class TestBatchCreationWithProxy {
    private static final Logger logger = LoggerFactory.getLogger(TestBatchCreationWithProxy.class);

    public static void main(String[] args) {
        logger.info("========== 测试使用 TCP 代理触发 Batch Creation 超时 ==========\n");
    ...
    private static void testWithProxy() {
        logger.info("【测试】TCP 代理方案");
        logger.info("策略: 通过代理控制网络流量\n");
        ...
        props.put("linger.ms", "10000");  // 10秒延迟，足够时间停止代理
        ...
        try {
            ...
            // 使用单独的 producer 发送预热消息（linger.ms=0，立即发送）
            ...
            warmupProducer.close();
            logger.info("预热完成，现在使用 linger.ms=10s 的 producer 发送测试消息\n");
            ...
            logger.warn("   消息已提交到 batch（linger.ms=10秒）");
            logger.warn("   现在立即停止代理转发，防止 batch 发送");
            logger.warn("\n");

            // 立即停止转发，在 linger.ms 延迟结束前
            logger.warn("🛑 停止代理转发...");
            KafkaTcpProxy.stopForwarding();
            logger.warn("✅ 代理已停止转发，batch 无法发送");
            logger.warn("   Batch 已创建但被阻塞，等待超时...");
